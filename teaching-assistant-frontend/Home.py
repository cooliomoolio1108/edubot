...
# debug_session_state()
# user = st.login("microsoft")

# if user:
#     st.write(f"Hello, {user['name']} ({user['email']}) 👋")
# else:
#     st.info("Please log in with your NTU account to continue.")
...
if "id_token" not in st.session_state:
    ...
    st.success(f"Logged in! User info: {st.session_state['id_token']}")
...
if "code" in query_params:
    ...
    if "id_token_claims" in result:
        ...
        tester = st.session_state["id_token"]
        print(st.session_state["id_token"])
        controller.set(name="test", value=tester)
        st.rerun()